...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow
    {
        ...
        public static void BeginZoomed(Rect rect, float zoom, float topPadding)
        {
            ...
            Vector4 padding = new Vector4(0, topPadding, 0, 0) * zoom;

            float x = ((rect.width * zoom) - rect.width) * -0.5f;
            float y = (((rect.height * zoom) - rect.height) * -0.5f) + (topPadding * zoom);

            float width = rect.width * zoom;
            float height = rect.height * zoom;

            GUI.BeginClip(new Rect(x, y, width, height));
        }
        ...
        public static void EndZoomed(Rect rect, float zoom, float topPadding)
        {
            ...
            float x = ((rect.width * zoom) - rect.width) * 0.5f;
            float y = (((rect.height * zoom) - rect.height) * 0.5f) + (-topPadding * zoom) + topPadding;
